...
public:
    TextBox(const std::shared_ptr<GraphicsContext> context, const std::string text, const std::string font, int ptsize, const SDL_Colour font_color, ALIGN_X alignX, ALIGN_Y alignY, SDL_Rect rect = SDL_Rect({0, 0, 0, 0}), SDL_Colour color = SDL_Colour({0, 0, 0, 0}), SDL_Colour border_color = SDL_Colour({0, 0, 0, 0}), bool hidden = false, int r = 0)
