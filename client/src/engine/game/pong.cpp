...
void Game::checkCollision() {
    if(this->ball.x <= this->lcl_goal.x && this->ball.y)

    // SDL_Rect lcl_goal { 0, 0.2*this->context->getHeight(), 5, 300 };
    // SDL_Rect rmt_goal { this->context->getWidth(), 0.2*this->context->getHeight(), 5, 300 };

    // bool goalX { bPos.x <= 800 && bPos.x >= 795 };
    // bool goalY { bPos.y <= 400 && bPos.y >= 0.2*this->context->getHeight() };

    // bool goalLX { bPos.x <= 5 && bPos.x >= 0 };
    // bool goalLY { bPos.y <= 400 && bPos.y >= 0.2*this->context->getHeight() };
